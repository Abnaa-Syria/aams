...
export default function LicenseDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { data } = await apiService.get(`/licenses/${id}`);
      setItem(data.data);
    } catch (err) {
      toast.error(err?.response?.data?.message || 'تعذر تحميل بيانات الرخصة');
      navigate('/licenses');
    } finally {
      setLoading(false);
    }
  ...
  const fileSrc = useMemo(() => resolveUploadUrl(item?.fileUrl), [item?.fileUrl]);
  const fileKind = useMemo(
    () => guessFileKind({ fileUrl: item?.fileUrl, fileName: item?.fileName }),
  ...
  return (
    <div className="page-container animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between flex-wrap gap-4 mb-8">
        <div>
          <h2 className="text-2xl font-black text-slate-800 tracking-tight">تفاصيل الرخصة</h2>
        ...
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={onDownload}
            disabled={!item.fileUrl || downloading}
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs font-bold text-slate-400">{item.fileName || '—'}</div>
                </div>
              </div>
            ...
            <div className="p-6">
              {!fileSrc ? (
                <div className="bg-slate-50 rounded-3xl p-10 text-center border border-slate-100">
                  ...
                  <div className="text-xs font-bold text-slate-400">يمكنك مراجعة بيانات الرخصة في القسم التالي</div>
                </div>
              ) : fileKind === 'image' ? (
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-3xl shadow-premium border border-slate-100 overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-100 flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-black text-slate-800">بيانات الرخصة</div>
                <div className="text-xs font-bold text-slate-400">كل المعلومات المتاحة عن الرخصة</div>
              </div>
            </div>

            <div className="px-6 py-2">
              <Field label="الموظف" value={item.user?.fullNameAr || '—'} />
              <Field label="النوع" value={typeLabels[item.type] || item.type || '—'} />
              <Field label="العنوان" value={item.title || '—'} />
              <Field label="رقم الرخصة" value={item.licenseNumber || '—'} />
              <Field label="تاريخ الإصدار" value={formatDate(item.issueDate)} />
              <Field label="تاريخ الانتهاء" value={formatDate(item.expiryDate)} />
              <Field label="الحالة" value={<StatusBadge status={item.status} />} />
              <Field label="اسم الملف" value={item.fileName || '—'} />
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-premium border border-slate-100 overflow-hidden">
            ...
            <div className="px-6 py-2">
              <Field label="تمت المراجعة بواسطة" value={item.reviewedBy || '—'} />
              <Field label="تاريخ المراجعة" value={formatDateTime(item.reviewedAt)} />
              <Field label="ملاحظات المراجعة" value={item.reviewNotes || '—'} />
              <Field label="تاريخ الإنشاء" value={formatDateTime(item.createdAt)} />
              <Field label="آخر تحديث" value={formatDateTime(item.updatedAt)} />
            </div>
          </div>
        </div>
      </div>
