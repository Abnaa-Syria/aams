...
export default function PenaltyDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { data } = await apiService.get(`/penalties/${id}`);
      setRow(data.data);
    } catch {
      toast.error('تعذر تحميل تفاصيل الجزاء');
      navigate('/penalties');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="page-container animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between flex-wrap gap-4 mb-8">
        <div>
          <h2 className="text-2xl font-black text-slate-800 tracking-tight">تفاصيل الجزاء</h2>
          <p className="text-xs font-bold text-slate-400 mt-1">عرض البيانات المرتبطة بالجزاء</p>
        </div>
        <button
          type="button"
